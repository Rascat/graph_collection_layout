...
def main():
    ...
    for iteration in ITERATION_VALUES:
        ...
        cmd += "-input {} -output {} ".format(INPUT, OUTPUT)
        cmd += "-vertices {} ".format(VERTICES)
        cmd += "-iterations {} ".format(iteration)
...
def build_params(config) -> None:
    ...
    if config.has_option(benchmark_section_name, 'iterations'):
        global ITERATION_VALUES
        ITERATION_VALUES = config.get(benchmark_section_name, 'iterations').split(',')
    else:
